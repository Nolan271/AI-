...
SCRIPT_GENERATION_PROMPT = """你是一个专业的视频解说词撰稿人。根据以下文档内容和用户需求，撰写一条完整的视频解说脚本。
...
3. 第一个场景是开场，最后一个场景是结尾/号召行动
4. 视觉风格必须严格遵循设计规范中的配色、字体和禁止项
5. 返回 JSON 数组，格式: [{{"index": 1, "start_time": 0, "duration": 22, "title": "场景标题", "narration_text": "解说文本", "visual_style": "风格描述", "visual_keywords": ["关键词1", "关键词2"]}}]"""
...
4. 包含音频元素（如果提供）
5. 时间线必须注册在 window.__timelines["main"]
6. 每个场景容器初始状态 opacity: 0，scene1 为 opacity: 1
7. 转场效果: scene N → N+1 在场景结束前 0.4s 开始淡出，下一场景立即显示
